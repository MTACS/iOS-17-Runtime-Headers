
@interface GEORPFeedbackFieldAnnotations : PBCodable <NSCopying> {
    struct { 
        unsigned int read_translatedTextField : 1; 
        unsigned int read_translatedTextListField : 1; 
        unsigned int wrote_anyField : 1; 
    }  _flags;
    PBDataReader * _reader;
    struct os_unfair_lock_s { 
        unsigned int _os_unfair_lock_opaque; 
    }  _readerLock;
    unsigned int  _readerMarkLength;
    unsigned int  _readerMarkPos;
    GEORPFeedbackTextField * _translatedTextField;
    GEORPFeedbackTextListField * _translatedTextListField;
}

@property (nonatomic, readonly) bool hasTranslatedTextField;
@property (nonatomic, readonly) bool hasTranslatedTextListField;
@property (nonatomic, retain) GEORPFeedbackTextField *translatedTextField;
@property (nonatomic, retain) GEORPFeedbackTextListField *translatedTextListField;

+ (bool)isValid:(id)arg1;

- (void).cxx_destruct;
- (void)copyTo:(id)arg1;
- (id)copyWithZone:(struct _NSZone { }*)arg1;
- (id)description;
- (id)dictionaryRepresentation;
- (bool)hasTranslatedTextField;
- (bool)hasTranslatedTextListField;
- (unsigned long long)hash;
- (id)init;
- (id)initWithData:(id)arg1;
- (id)initWithDictionary:(id)arg1;
- (id)initWithJSON:(id)arg1;
- (bool)isEqual:(id)arg1;
- (id)jsonRepresentation;
- (void)mergeFrom:(id)arg1;
- (void)readAll:(bool)arg1;
- (bool)readFrom:(id)arg1;
- (void)setTranslatedTextField:(id)arg1;
- (void)setTranslatedTextListField:(id)arg1;
- (id)translatedTextField;
- (id)translatedTextListField;
- (void)writeTo:(id)arg1;

@end
